...
validate_value_options "--asm-ovlp --cns-cov --cns-fast --cns-sensitive --genome-size --jobs --min-read-len --necat --num-iter --outdir --ovlp-fast --ovlp-sensitive --polish --prep-cov --reads --threads --trim-ovlp -g -j -n -o -r -t" "$@"
...
[[ -z "${MAX_JOBS}" ]] || positive_int MAX_JOBS "${MAX_JOBS}"
...
READSDIR=$(realpath "$READSDIR")
command -v "$NECAT_CMD" >/dev/null || { echo "ERROR: NECAT not found" >&2; exit 1; }
PIDS=()
...
for pid in "${PIDS[@]}"; do
  wait "$pid" || echo "worker $pid" >> "$FAILURES_FILE"
done
