...
validate_value_options "--cmdfile --input --outdir --script --threads -c -i -o -s -t" "$@"

RAW_DIR="./result/assembly/denovo/"
OUT_DIR="./result/clean/denovo"
SCRIPT="$SCRIPT_DIR/lib/remove_redundant_contigs.py"
THREADS=32
...
Options:
  ...
  -s, --script PATH      Redundancy-removal script
...
Example:
  $(basename "$0") \\
    ...
    -s ./workflow/lib/remove_redundant_contigs.py \\
    -t 32
...
for fasta in "${FASTA_FILES[@]}"; do
  ...
  out="$OUT_DIR/$name"
  printf "%q %q -d %q -o %q\n" python3 "$SCRIPT" "$fasta" "$out" >> "$COMMANDS"
done
