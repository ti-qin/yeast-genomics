#!/usr/bin/env python3
# Adapted from https://github.com/HaploTeam/1086YeastGenomes.
# -*- coding: utf8 -*-
#----------------------------------------------------------------------------
# Created By  : vloegler
...
(80% by default threshold) by other contigs of the draft assembly, the contig is removed.
...
def blastn(fasta1, fasta2, blastPath, out):
	command = [blastPath + "blastn", "-query", fasta1, "-subject", fasta2, "-outfmt", "6", "-out", out]
	print(command)
...
parser.add_argument("--coverage-threshold", type=float, default=80.0,
                    help="Redundancy coverage percentage (default: 80, preserved from supplied code)")

# Read arguments from the command line
args = parser.parse_args()
if not 0 < args.coverage_threshold <= 100:
	parser.error("--coverage-threshold must be in (0, 100]")
...
if not len(draftFasta) or any(not seq.seq or not seq.seq.strip("Nn") for seq in draftFasta):
	raise SystemExit("ERROR: Assembly contains no records or an empty/all-N contig")
if len(set(draftFasta.getID())) != len(draftFasta):
	raise SystemExit("ERROR: Duplicate FASTA sequence identifiers")
...
with open(blastResultsPath) as blastResultsFile:
	...
	for row in blastResults:
		...
		if contig1 != contig2:
			...
			startPos = min(int(row[6]), int(row[7]))
			endPos = max(int(row[6]), int(row[7])) + 1
...
# While there is contigs covered on more than 80% by default, remove contig and recompute the whole process
contigsRemoved = []
contigsRemovedCoverage = []
while overlap and max(overlap) >= args.coverage_threshold:
